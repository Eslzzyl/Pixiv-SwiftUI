...
public enum DoubleTap: Equatable {
    case disabled
    case scale(CGFloat)
}
...
public enum ZoomConfig: Equatable {
    case disabled
...
public struct Config<Element> {
    ...
    /// Binding for the raw vertical dismiss progress, from 0 to 1.
    public var dismissProgress: Binding<CGFloat>?
    ...
    /// Optional token used to avoid rebuilding hosted pages for unrelated view updates.
    public var reloadID: AnyHashable?
...
public extension LazyPager {
    func onDismiss(
        ...
        this.config.dismissProgress = dismissProgress
    ...
    func reloadID(_ id: AnyHashable) -> LazyPager {
        var this = self
        this.config.reloadID = id
        return this
    }
}
...
extension LazyPager: UIViewControllerRepresentable {
    ...
    public func updateUIViewController(_ uiViewController: ViewDataProvider<Content, DataCollecton, Element>, context: Context) {
        let shouldReloadViews = config.reloadID == nil || uiViewController.reloadID != config.reloadID
